...
if __name__ == "__main__":
    ...
    for i in range(0, 150):
        fixed_param = {
            ...
            'max_depth': 6,  # [3, 5, 6, 7, 9, 12, 15, 17, 25]#5
            'learning_rate': 0.05,  # 0.05
            "colsample_bytree": 0.6,#0.33
            'subsample': 0.6,  # 0.33
            "eval_metric": 'mae',  # mse,
            "gpu_id": 2,
    ...
    mse_lists = pd.DataFrame({
        'test_mse': test_list_mse
    })
    mse_lists.to_csv('../checkpoints/'+name+'/mse1-150_' + str(mean) + ".csv", index=False)
